...
public interface SubotaRepository extends JpaRepository<Subota, Integer> {
	...
	List<Subota> findAll(int id);
		...
		@Query(value="select * from subota p inner join odeljenja o on(p.id_odeljenje=o.id_odeljenje) inner join korisnici k on(k.id_korisnik=o.ucitelj_id_ucitelj) where k.id_korisnik = ?1", nativeQuery = true)
